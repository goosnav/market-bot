...
def hydrate_campaign_row(row: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "dispatched_count": int(row.get("dispatched_count") or 0),
...
def hydrate_preview_row(row: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "next_attempt_at": string_value(row.get("next_attempt_at")),
        "last_error_code": string_value(row.get("last_error_code")),
        "last_error_detail": string_value(row.get("last_error_detail")),
        "dead_lettered_at": string_value(row.get("dead_lettered_at")),
